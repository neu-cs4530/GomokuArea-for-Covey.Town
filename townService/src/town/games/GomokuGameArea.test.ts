...
import * as GomukuGameModule from './GomokuGame';
import Game from './Game';
import GomokuGameArea from './GomokuGameArea';

class TestingGame extends Game<GomokuGameState, GomokuMove> {
  public constructor() {
    super({
      moves: [],
...
describe('GomokuGameArea', () => {
  let gameArea: GomokuGameArea;
  let player1: Player;
  let player2: Player;
  let interactableUpdateSpy: jest.SpyInstance;
  let game: TestingGame;
  beforeEach(() => {
    const gameConstructorSpy = jest.spyOn(GomukuGameModule, 'default');
    ...
    describe('[T3.2] when given a GameMove command', () => {
      it('should throw an error when there is no game in progress', () => {
        expect(() =>
          gameArea.handleCommand(
            { type: 'GameMove', move: { col: 0, row: 0, gamePiece: 'Black' }, gameID: nanoid() },
            player1,
          ),
      ...
      describe('when there is a game in progress', () => {
        ...
        it('should dispatch the move to the game and call _emitAreaChanged', () => {
          const move: GomokuMove = { col: 0, row: 0, gamePiece: 'Black' };
        ...
        describe('when the game is over, it records a new row in the history and calls _emitAreaChanged', () => {
          ...
          test('when White wins', () => {
            const move: GomokuMove = { col: 0, row: 0, gamePiece: 'White' };
            jest.spyOn(game, 'applyMove').mockImplementationOnce(() => {
